...
<div style="display:flex;gap:0;margin-bottom:28px;border-radius:12px;overflow:hidden;border:1px solid var(--color-border);">
  <div id="step1-indicator" style="flex:1;padding:12px;text-align:center;font-size:13px;font-weight:600;background:var(--color-primary);color:#fff;">
    <i class="bi bi-1-circle-fill"></i> Kurs Bilgileri
  </div>
  <div id="step2-indicator" style="flex:1;padding:12px;text-align:center;font-size:13px;font-weight:600;background:var(--color-surface);color:var(--color-text-muted);">
...
<div id="step1" class="plain-card">
  <h2 style="font-size:1.1rem;margin-bottom:20px;"><i class="bi bi-info-circle"></i> Kurs Bilgileri</h2>
  ...
  <div class="row g-3">
    ...
    <div class="col-md-6">
      ...
      <select class="form-control" id="courseCat" name="courseCat" style="appearance:auto;">
        ...
        <option value="<%: cat.CategoryId %>" <%= cat.CategoryId.ToString() == Request.Form["courseCat"] ? "selected" : "" %>><%: cat.Name %></option>
        <% } %>
      </select>
    </div>
    ...
    <div class="col-md-6">
      ...
      <select class="form-control" id="courseLevel" name="courseLevel" style="appearance:auto;">
        <option value="Başlangıç" <%= Request.Form["courseLevel"] == "Başlangıç" ? "selected" : "" %>>Başlangıç</option>
        <option value="Orta Seviye" <%= Request.Form["courseLevel"] == "Orta Seviye" ? "selected" : "" %>>Orta Seviye</option>
        <option value="İleri Seviye" <%= Request.Form["courseLevel"] == "İleri Seviye" ? "selected" : "" %>>İleri Seviye</option>
      </select>
    </div>
    ...
    <div class="col-md-6">
      ...
      <div style="display:flex;gap:10px;align-items:center;">
        <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;border:1px solid var(--color-border);border-radius:8px;cursor:pointer;" onclick="toggleFree(true)" id="freeOpt">
          <input type="radio" name="priceType" value="free" id="radioFree" <%= (Request.Form["priceType"]=="free" || string.IsNullOrEmpty(Request.Form["priceType"])) ? "checked" : "" %> style="cursor:pointer;" />
        ...
        <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;border:1px solid var(--color-border);border-radius:8px;cursor:pointer;" onclick="toggleFree(false)" id="paidOpt">
          <input type="radio" name="priceType" value="paid" id="radioPaid" <%= Request.Form["priceType"]=="paid" ? "checked" : "" %> style="cursor:pointer;" />
          <label for="radioPaid" style="cursor:pointer;margin:0;font-size:14px;"><i class="bi bi-tag"></i> Ücretli</label>
        </div>
      </div>
    </div>

    <div class="col-md-6" id="priceField" style="<%= Request.Form["priceType"]=="paid" ? "" : "display:none;" %>">
      <label class="form-label">Fiyat (₺)</label>
      <input class="form-control" id="coursePrice" name="coursePrice" type="number" min="0" step="1"
             placeholder="Örn: 999" value="<%: Request.Form["coursePrice"] %>" />
    </div>
  ...
  <div class="d-flex justify-content-between mt-4">
    ...
    <button type="button" class="btn btn-primary-custom" onclick="goToStep2()">
      Devam: Ders Ekle <i class="bi bi-arrow-right"></i>
    </button>
  </div>
</div>
  ...
  <div class="plain-card" style="padding:0;overflow:hidden;">
    ...
    <div class="table-responsive">
      <table class="table table-hover" style="margin:0;">
        <thead><tr><th style="padding:12px 16px;">#</th><th>Başlık</th><th>Süre</th><th>Önizleme</th></tr></thead>
...
<script>
  ...
  (function(){
    ...
    if (hasCourse) {
      ...
      document.getElementById('step1-indicator').style.background = 'var(--color-surface)';
      document.getElementById('step1-indicator').style.color = 'var(--color-text-muted)';
      document.getElementById('step2-indicator').style.background = 'var(--color-primary)';
